...
/**
 * Campo Matéria em destaque
 */
// Adicionar uma metabox com o campo personalizado "materia_destaque" ao tipo de post personalizado "noticias-climatempo"
function adicionar_metabox_materia_destaque() {
    add_meta_box(
        'materia_destaque_metabox',
        'Matéria Destaque',
        'exibir_metabox_materia_destaque',
        'noticias-climatempo',
        'normal',
        'high'
    );
}
add_action('add_meta_boxes', 'adicionar_metabox_materia_destaque');

// Função para exibir o conteúdo da metabox
function exibir_metabox_materia_destaque($post) {
    $materia_destaque = get_post_meta($post->ID, 'materia_destaque', true);
    
    // Verificar o número de posts com o campo "materia_destaque" como verdadeiro
    ...
    $num_posts_materia_destaque = $query->post_count;
    
    // Verificar se o post atual já tem o campo "materia_destaque" como verdadeiro
    $current_post_has_materia_destaque = ($materia_destaque == true);
    
    // Desabilitar o input do checkbox se já existirem 3 posts com a matéria em destaque
    // e o post atual não tem o campo "materia_destaque" como verdadeiro
    $disabled = ($num_posts_materia_destaque >= 3 && !$current_post_has_materia_destaque) ? 'disabled' : '';
    
    ?>
    <div style="display: flex; flex-direction: column; padding: 10px 0; width: 100%; height: 80px; justify-content: space-between">
        <label for="materia_destaque_checkbox">
            <input type="checkbox" id="materia_destaque_checkbox" name="materia_destaque" <?php echo $materia_destaque ? 'checked' : ''; ?> <?php echo $disabled; ?> />
            Destacar matéria
        </label>
        <span>Quantidade máxima de posts que pode ter a matéria em destaque é: <strong> 3 </strong></span>
        <span>Quantidade total de posts com matéria em destaque atualmente: <strong> <?php echo $num_posts_materia_destaque ?> </strong> </span>
    </div>
    <?php
}
